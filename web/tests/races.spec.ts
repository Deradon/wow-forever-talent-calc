...
test('a race page shows its traits, their verdicts and the source once', async ({ page }) => {
  ...
  await expect(page.getByTestId('race-trust')).toContainText('has been checked by hand yet')
  await expect(page.getByTestId('race-trust')).not.toContainText('not yet reviewed')
