...
| `npm test` | Vitest: rules engine table, codec, routing, Zod-vs-JSON-Schema, `validate-data`. |
| `npm run e2e` | Playwright smoke test (builds with `build:e2e`, then `preview`). First time: `npx playwright install chromium`. |
| `npm run lint` | oxlint. |
