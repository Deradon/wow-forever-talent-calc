...
| `spells-baseline.json` | Classic Era trainer-taught spell **names** per class plus a `_shared` bucket for the General page rows. No ranks, no tooltip text, no talent abilities, no racials. **Written from memory by the data lead, not retrieved from anywhere**; `"verified": false`. Used by `pipeline/stages/11_spellbook.py build` to decide whether a Forever spell name is new. Caveats and the replacement recipe: `spells-baseline.md`. | hand-written 2026-09-13, unverified |
